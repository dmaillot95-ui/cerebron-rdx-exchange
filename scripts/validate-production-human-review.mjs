...
const errors=[];
for(const f of [reviewPath,proofPath,proposalPath]) if(!fs.existsSync(f)) errors.push('missing '+f);
...
if(errors.length){errors.forEach(e=>console.error('FAIL '+e));console.error('RDX Production Human Review Gate: FAIL');process.exit(1)}
console.log('RDX Production Human Review Gate: PASS decision='+review.decision+' review='+review.review_sha256);
