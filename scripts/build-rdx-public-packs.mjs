...
for(const file of files){
  ...
  const publicPack={
    schema_version:'RDX-PUBLIC-PACK-1.0',
    ...
    claims:(p.claims||[]).map(c=>({id:c.id,evidence_level:c.evidence_level,statement:c.statement})),
  ...
  const claims=publicPack.claims.map(c=>`<div class="claim"><b>${esc(c.id)} · E${esc(c.evidence_level)}</b><div>${esc(c.statement)}</div></div>`).join('');
  const sources=publicPack.sources.map(s=>`<div class="source"><b>${esc(s.id)}</b> · ${esc(s.author_or_org)}<br><a href="${esc(s.url)}" rel="noreferrer">${esc(s.title)}</a><div class="muted">${esc(s.source_type)} · licence: ${esc(s.license_status)}</div></div>`).join('');
  const work=publicPack.open_work.map(x=>`<li>${esc(x)}</li>`).join('');
  const html=`<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(p.id)} — Fiche de preuve</title><style>:root{--bg:#070a12;--panel:#101624;--text:#f3f6fb;--muted:#9ca9bd;--line:#263247;--aqua:#6ee7d8;--amber:#f2c66d;--max:1080px}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:16px/1.6 system-ui,sans-serif}.wrap{width:min(var(--max),calc(100% - 32px));margin:auto}.nav{border-bottom:1px solid var(--line);padding:18px 0}.nav a,a{color:var(--aqua)}.hero{padding:62px 0 34px}.eyebrow{color:var(--aqua);font-weight:800}.grid{display:grid;grid-template-columns:1.2fr .8fr;gap:22px;padding:26px 0 60px}.panel{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:24px}.claim,.source{padding:14px 0;border-top:1px solid var(--line)}.muted{color:var(--muted)}.warning{border-left:4px solid var(--amber);padding:14px 16px;background:#171613}.pill{display:inline-block;border:1px solid var(--line);padding:7px 11px;border-radius:999px;margin:4px}@media(max-width:800px){.grid{grid-template-columns:1fr}}</style></head><body><div class="nav"><div class="wrap"><a href="/">← CÉRÉBRON R&D EXCHANGE</a></div></div><main><section class="hero"><div class="wrap"><div class="eyebrow">${esc(p.id)} · fiche de preuve</div><h1>${esc(p.title)}</h1><p>${esc(p.decision_question)}</p><span class="pill">${esc(p.status)}</span><span class="pill">E${maxEvidence} max</span></div></section><div class="wrap"><div class="warning"><strong>Claim ceiling :</strong> ${esc(publicPack.claim_ceiling)}</div><div class="grid"><div class="panel"><h2>Claims</h2>${claims}</div><aside><div class="panel"><h2>Travaux ouverts</h2><ul>${work}</ul></div></aside></div><div class="panel"><h2>Sources</h2>${sources}</div></div></main></body></html>`;
